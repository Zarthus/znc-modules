...
public:
    ...
    EModRet OnPrivCTCP(CNick& Nick, CString& sMessage) {
        ...
        if (IsCtcpNotifier(sMessage.Token(0)))
            PutModule("CTCP " + sMessage.Token(0) + " requested by " + Nick.GetNick() + " (" + Nick.GetHostMask() + ").");
    ...
    EModRet OnChanCTCP(CNick& Nick, CChan& Channel, CString& sMessage) {
        if (IsCtcpNotifier(sMessage.Token(0)))
            PutModule("CTCP " + sMessage.Token(0) + " requested by " + Nick.GetNick() + " (" + Nick.GetHostMask() + ") to " + Channel.GetName() + ".");
    ...
    void ShowAllCtcp(const CString& sCtcp) {
        ...
        if (GetNV("ctcp_showall").empty()) {
            ...
            PutModule("Will now show all incoming CTCP requests.");
        } else {
            DelNV("ctcp_showall");
            PutModule("Will no longer show all incoming CTCP requests.");
        }
    }
...
USERMODULEDEFS(CCtcpNotifierMod, "Make ZNC notify you when you receive a CTCP request in your CTCPNotifier list.")
